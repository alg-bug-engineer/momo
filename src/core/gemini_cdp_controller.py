...
class GeminiCDPController(BrowserController):
    ...
    async def send_message(self, query: str):
        """在输入框输入文本并发送"""
        self.logger.debug(f"准备发送消息: {query}")
        ...
        try:
            ...
            # 输入文本
            self.logger.debug(f"输入文本: {query}")
            await self.page.type(selector, query, delay=50)
            
            # 按回车发送
            self.logger.debug("按回车发送...")
    ...
    async def wait_for_images_generated(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug("✓ 检测到至少一张图片已加载")
            except:
            ...
            if await wait_for_images_loading(
                ...
                self.logger.debug("✓ 图片加载完成")
                return True
            else:
                self.logger.warning("图片加载超时")
                return False
        ...
        try:
            ...
            if use_create_images and save_images:
                if await self.wait_for_images_generated():
                    ...
                    if saved_files:
                        self.logger.debug(f"✓ 成功保存 {len(saved_files)} 张图片到 {images_dir}")
                    else:
                        self.logger.warning("未保存任何图片")
                else:
                    self.logger.warning("图片生成超时或失败")
